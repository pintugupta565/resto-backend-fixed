...
// test-conn start
const db = mysql.createPool({
  host: process.env.DATABASE_HOST || 'mysql.psgwebsolution.in',
  user: process.env.DATABASE_USER || 'psgwebsolution_user',
  password: process.env.DATABASE_PASSWORD || '',
  database: process.env.DATABASE_NAME || 'psgwebsolution_restodb',
  waitForConnections: true,
  connectionLimit: 10,
  queueLimit: 0
});
